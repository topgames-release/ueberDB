 ...
 */
 ...
 exports.database.prototype.init = function (callback) {
   ...
   var sqlCreate = "CREATE TABLE IF NOT EXISTS `store_eptherpad` ( " +
   ...
   var sqlAlter = "ALTER TABLE store_eptherpad MODIFY `key` VARCHAR(100) COLLATE utf8mb4_bin;";
   ...
   db.query(sqlCreate, [], function (err) {
     ...
     var tableCharSet = "SELECT CCSA.character_set_name FROM information_schema.`TABLES` T,information_schema.`COLLATION_CHARACTER_SET_APPLICABILITY` CCSA WHERE CCSA.collation_name = T.table_collation AND T.table_schema = '" + db.database + "' AND T.table_name = 'store_eptherpad'";
 ...
 exports.database.prototype.get = function (key, callback) {
   this.db.query("SELECT `value` FROM `store_eptherpad` WHERE `key` = ? AND BINARY `key` = ?", [key, key], function (err, results) {
     var value = null;
 ...
 exports.database.prototype.findKeys = function (key, notKey, callback) {
   var query = "SELECT `key` FROM `store_eptherpad` WHERE `key` LIKE ?"
     , params = []
     ;
 ...
 exports.database.prototype.set = function (key, value, callback) {
   if (key.length > 100) {
     ...
     this.db.query("REPLACE INTO `store_eptherpad` VALUES (?,?)", [key, value], function (err, info) {
       callback(err);
     });
   }
 ...
 exports.database.prototype.remove = function (key, callback) {
   this.db.query("DELETE FROM `store_eptherpad` WHERE `key` = ? AND BINARY `key` = ?", [key, key], callback);
 ...
 exports.database.prototype.doBulk = function (bulk, callback) {
   ...
   var replaceSQL = "REPLACE INTO `store_eptherpad` VALUES ";
   ...
   var removeSQL = "DELETE FROM `store_eptherpad` WHERE `key` IN " + keysToDelete + " AND BINARY `key` IN " + keysToDelete + ";";
